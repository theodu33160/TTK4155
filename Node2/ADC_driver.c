...
//Function to initialize all the registers for proper working of ADC
void ADC_init()
{
...
// Function that sets up a conversion in order to read any value of the channels of the ADC
uint16_t ADC_read()
{
	uint16_t ADC_data = 0;
	...
	while( ! (ADCSRA && (1<<ADIF)) );
	//if we dont use interrupt clear the ADIF bit with a 1
